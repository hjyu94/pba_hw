...
bool Context::Init() {
    ...
    m_simpleProgram = Program::Create(std::string(CURRENT_SOURCE_DIR) + std::string("/shader/simple.vs"), std::string(CURRENT_SOURCE_DIR) + std::string("/shader/simple.fs"));
    if (!m_simpleProgram)
        return false;

    m_program = Program::Create(std::string(CURRENT_SOURCE_DIR) + std::string("/shader/lighting.vs"), std::string(CURRENT_SOURCE_DIR) + std::string("/shader/lighting.fs"));
    if (!m_program)
        return false;

    SPDLOG_INFO("program id: {}", m_program->Get());

    m_model = Model::Load(std::string(CURRENT_SOURCE_DIR) + std::string("/model/backpack.obj"));
    if (!m_model)
        return false;
